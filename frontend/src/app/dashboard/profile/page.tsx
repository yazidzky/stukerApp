...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        console.error("Error auto-refreshing user rating:", error);
      }
    }, 30000); // 30 detik
    ...
    setEditedData((prev) => {
      ...
      if (field === "password") {
        ...
        if (!value.match(/^\*+$/)) {
          ...
          localStorage.setItem("passwordLength", value.length.toString());
  ...
  const handleSwitch = async () => {
    try {
      // Pastikan user memiliki role stuker terlebih dahulu
      if (!user?.role?.includes("stuker")) {
        await switchRole("stuker");
      }
      // Redirect ke dashboard stuker setelah switch role berhasil
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Gagal beralih ke Stuker. Silakan coba lagi.";
  ...
  return (
    <div className="px-4 flex flex-col">
      ...
      {/* 🔸 Rating pengguna (sebagai user) */}
      <Rating
        rating={user?.avgRatingAsUser ? Number(user.avgRatingAsUser) * 10 : 0}
        reviews={user?.countRatingAsUser ? Number(user.countRatingAsUser) : 0}
      />
